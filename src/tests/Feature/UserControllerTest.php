...
class UserControllerTest extends TestCase
{
    ...
    public function it_displays_existing_user_profile_data_on_the_edit_page(): void
    {
        ...
        // --- 実行 (Act) ---
        // 作成したユーザーとしてログインし、プロフィール編集ページにGETリクエストを送信する
        ...
        // 2. ユーザー情報が、フォームのvalue属性に正しく設定されていることを確認
        $response->assertSee('value="テストユーザー名"', false);
        $response->assertSee('value="123-4567"', false);
        $response->assertSee('value="東京都テスト区テスト町1-2-3"', false);

        // 3. プロフィール画像が、imgタグまたはstyle属性で正しく表示されていることを確認
        //    (ユーザーさんの実装に合わせて、どちらか、または両方をテストします)
        //    例：背景画像の場合
